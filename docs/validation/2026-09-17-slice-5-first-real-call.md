...
- llm-once没有调用前缓存，每次执行都可能计费。只供显式开发小样本使用。
- 每次最多一次调用、0 retry、8页/24000字符、1024默认输出Token（最大4096）；
  字符/字节预算不是精确输入Token或货币预算。
- estimated_cost为null，pricing_not_configured；不把未知费用写成0。
- Task 3还需调用前缓存、明确价格/成本上限、retry策略和中断恢复。
