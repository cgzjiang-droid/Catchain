...
def invoke(tmp_path, monkeypatch, body):
    calls = []
...
def test_provider_and_workflow_preserve_response_usage_and_trusted_run(tmp_path, monkeypatch):
    path, calls = invoke(tmp_path, monkeypatch, api_body(candidate()))
    assert len(calls) == 1
    ...
    reply = json.loads((path.parent / "response.json").read_text())
    assert reply["input_tokens"] == 50 and reply["output_tokens"] == 30
    assert result["configuration"]["estimated_cost"] is None
    assert "secret-test-key" not in "".join(p.read_text() for p in path.parent.glob("*.json"))
...
def test_auth_error_is_sanitized_and_not_retried(tmp_path, monkeypatch):
    ...
    assert len(calls) == 1
    assert "secret-test-key" not in next(tmp_path.glob("*/failed.json")).read_text()
